...
class NoduleClassifyInference(InferenceModel):
    ...
    def forward(self, x):
        ...
        net_out = self.networks.classification(_input)
        return net_out
    ...
    def _inference(self, vol, bboxes, spacing, batch_size=5, *args, **kwargs):
        ...
        for bbox in bboxes:
            ...
            new_coord = coord / self.new_spacing * spacing
            new_diameter = [diameter / self.new_spacing[0] * spacing[0]]
...
if __name__ == '__main__':
    config_path = '/host/med-lightning-deploy/models/nodule_classification/model_concat.yaml'
    pretrain_weights = ['/host/med-lightning-deploy/models/nodule_classification/weight.ckpt']
    classifier = NoduleClassifyInference(config_path, pretrain_weights, 2)
